...
void MockRegistry::registry_global(uint32_t name, const QString &interface, uint32_t version)
{
    if (interface == QLatin1String("wl_compositor")) {
    ...
    } else if (interface == QLatin1String("wl_shm")) {
        ...
        wlrLayerShell = new MockWlrLayerShellV1(object(), name, qMin<uint32_t>(version, 2));
    ...
    } else if (interface == QLatin1String("zwlr_screencopy_manager_v1")) {
        wlrScreencopyManager = new MockWlrScreencopyManagerV1(object(), name, qMin<uint32_t>(version, 2));
    }
}
